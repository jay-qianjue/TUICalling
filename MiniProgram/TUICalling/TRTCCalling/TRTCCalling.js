...
class TRTCCalling {
	constructor(params) {
		...
		this.TSignalingClient = new TSignalingClient({
			TSignaling: wx.$TSignaling
		});
		this.TRTCCallingDelegate = new TRTCCallingDelegate({
	...
	handleCallMode(callType, callModeMessage) {
		...
		this.TRTCCallingDelegate.onCallMode({
			...
			message: callModeMessage.data.message
		});
	...
	handleNewInvitationReceived(event) {
		...
		if (inviteData && inviteData.data && inviteData.data.cmd === 'hangup') {
	...
	async handleInviteeAccepted(event) {
		...
		if (event.data.inviter === this._getUserID() && this.data.callStatus === CALL_STATUS.CALLING) {
			...
			this.TRTCCallingDelegate.onUserAccept(
				event.data.invitee,
				await this.getUserProfile(this.data._unHandledInviteeList.map(item => ({
					userID: item
				}))),
			);
		}
	...
	handleInviteeRejected(event) {
		...
		if (oldLineBusy || newLineBusy) {
			...
			this.TRTCCallingDelegate.onReject({
				inviteID: event.data.inviteID,
				invitee: event.data.invitee
			});
		}
		if (this._getGroupCallFlag()) {
			this._setUnHandledInviteeList(event.data.invitee, (list) => {
				...
				if (list.length === 0) {
					...
					const isPlayer = this.data.callStatus === CALL_STATUS.CONNECTED && this.data.playerList
						.length === 0;
	...
	handleInvitationTimeout(event) {
		...
		const {
			groupID = '', inviteID, inviter, inviteeList, isSelfTimeout
		} = event.data;
	...
	async onRemoteAudioAdd(event) {
		...
		this.setPlayerAttributesHandler(players[0], {
			muteAudio: false
		});
	}
	...
	exitRoom(callEnd) {
		...
		if (this.data.isCallEnd) {
			this.TRTCCallingDelegate.onCallEnd({
	...
	async setPlayerAttributesHandler(player, options) {
		...
		this.data.playerList =
			playerList.length > 0 ? this._updateUserProfile(this.data.playerList, playerList) : this.data
			.playerList;
		this.TRTCCallingDelegate.onUserUpdate({
			pusher: this.data.pusher,
			playerList: this.data.playerList
	...
	_pusherAudioHandler() {
		if (this.data.pusher.enableMic) {
			...
			this.TRTC.getPusherInstance().setMICVolume({
				volume: 0
			});
		} else {
			this.setPusherAttributesHandler({
	...
	async call(params) {
		...
		try {
			const res = await this.TSignalingClient.invite({
				roomID,
				...params
			});
	...
	async groupCall(params) {
		...
		try {
			...
			const res = await this.TSignalingClient.inviteGroup({
				roomID,
				...params
			});
			if (!params.groupID) {
	...
	async lastOneHangup(params) {
		...
		const res = await this.TSignalingClient.lastOneHangup({
			isGroup,
	...
	async getUserProfile(userList, type = 'array') {
		...
		const imResponse = await this.tim.getUserProfile({
			userIDList: list
		});
		let result = null
		switch (type) {
